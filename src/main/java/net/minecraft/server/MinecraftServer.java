...
//        int i;
        ...
        if(this.ticks%loggingTick == 0) {
            log.log(Level.INFO, logPrefix + ((float)onTicktime/(loggingTick)) + " ms on tick() " + ((float)onEntitytime/(loggingTick)) + " ms on updateEntities(). " + ((float)onNetworktime/(loggingTick)) + " ms on Network. " + ((float)onPlayerupdatetime/(loggingTick)) + " ms on Playerupdate. " + Math.min(((float)1000/((float)(tickDuration)/loggingTick)),20) + " TPS. " + s.players.size() + " Players. " + ((float)1000/((float)(tickDuration)/loggingTick)) + " potential TPS.");
            onTicktime = 0;
            onEntitytime = 0;
            onNetworktime = 0;
