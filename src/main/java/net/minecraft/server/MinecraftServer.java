...
//        int i;
        ...
        for (int i = 0; i < this.worlds.size(); ++i) {
            ...
            // if (i == 0 || this.getAllowNether()) {
                ...
                onPlayerupdatetime += worldserver.timings.tracker.getCurTickTotal(); //Hydrox
                this.methodProfiler.b();
                this.methodProfiler.b();
            // } // CraftBukkit
        ...
        onNetworktime += SpigotTimings.connectionTimer.getCurTickTotal(); //Hydrox
        ...
        if(this.ticks%loggingTick == 0) {
            ...
            h.info(logPrefix + "callEvent() times");
            h.info(logPrefix + "Sum: " + timeSummary+ " ms.");
            for (int i=0; i<5; i++) {
                if (topArray[i] == null) {
                    continue;
                }
                ...
                h.info(logPrefix + topArray[i] + ": " + timeOnPlugin + " ms. (" + (timeOnPlugin/timeSummary)*100 + "%)");
                times.put(topArray[i], 0L);
            }

            for (WorldServer world : this.worlds) {
                h.info(logPrefix + "loaded Chunks/Entities/Players for " + world.getWorld().getName() + ": " + world.chunkProviderServer.getLoadedChunks() + "/" + world.entityList.size() + "/" + world.players.size());
	    }
        }
        this.methodProfiler.b();
    }
