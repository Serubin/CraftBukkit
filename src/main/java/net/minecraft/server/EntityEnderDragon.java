...
public class EntityEnderDragon extends EntityLiving implements IComplex {
    ...
    private void i() {
        ...
        if (this.random.nextInt(2) == 0 && !this.world.players.isEmpty()) {
            ...
            EntityTargetLivingEntityEvent event = CraftEventFactory.callEntityTargetLivingEvent(this, (EntityPlayer) this.bS, EntityTargetEvent.TargetReason.RANDOM_TARGET);
            if (event.isCancelled()) {
        	this.bS = null;
            }
        } else {
        ...
        for (int k1 = i; k1 <= l; ++k1) {
            for (int l1 = j; l1 <= i1; ++l1) {
                for (int i2 = k; i2 <= j1; ++i2) {
                    ...
                    if (j2 != 0) {
                        if (j2 != Block.OBSIDIAN.id && j2 != Block.WHITESTONE.id && j2 != Block.BEDROCK.id && j2 != Block.DRAGON_EGG.id) {
                            flag1 = true;
