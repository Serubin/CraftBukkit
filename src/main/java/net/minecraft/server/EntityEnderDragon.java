...
public class EntityEnderDragon extends EntityLiving implements IComplex {
                ...
                if (this.bT != null) {
                    ...
                    if (d6 < 60.0F) {
                        ...
                        if (this.attackTicks == 0) {
                            ++this.fireballCount;
                            if (this.fireballCount == 1) {
                                this.attackTicks = 60;
                            } else if (this.fireballCount <= 10) {
                                this.attackTicks = 3;
                            } else {
    ...
    private void i() {
        ...
        if (this.random.nextInt(2) == 0 && !this.world.players.isEmpty()) {
            ...
            EntityTargetLivingEntityEvent event = CraftEventFactory.callEntityTargetLivingEvent(this, (EntityPlayer) this.bT, EntityTargetEvent.TargetReason.RANDOM_TARGET);
            if (event.isCancelled()) {
        	this.bT = null;
            }
        } else {
        ...
        for (int k1 = i; k1 <= l; ++k1) {
            for (int l1 = j; l1 <= i1; ++l1) {
                for (int i2 = k; i2 <= j1; ++i2) {
                    ...
                    if (j2 != 0) {
                        if (j2 != Block.OBSIDIAN.id && j2 != Block.WHITESTONE.id && j2 != Block.BEDROCK.id && j2 != Block.DRAGON_EGG.id && this.world.getGameRules().getBoolean("mobGriefing")) {
